-- Script Date: 10/15/2019 3:17 PM  - ErikEJ.SqlCeScripting version 3.5.2.81
SELECT 1;
PRAGMA foreign_keys=OFF;
BEGIN TRANSACTION;
CREATE TABLE [NutrientBulletPoints] (
  [Id] INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL
, [NutrientName] ntext NULL
, [BulletPoint] ntext NULL
, [Range] ntext NULL
);
CREATE TABLE [Horses] (
...
, [Vanadium] float NOT NULL
, [Molybdenum] float NOT NULL
, [Boron] float NOT NULL
, [Iodine] float NOT NULL
, [Lithium] float NOT NULL
, [Phosphorus] float NOT NULL
, [Selenium] float NOT NULL
, [Strontium] float NOT NULL
, [Sulfur] float NOT NULL
, [Cobalt] float NOT NULL
, [Iron] float NOT NULL
, [Germanium] float NOT NULL
, [Rubidium] float NOT NULL
, [Zirconium] float NOT NULL
, [Aluminum] float DEFAULT (0.0000000000000000e+000) NOT NULL
, [Antimony] float DEFAULT (0.0000000000000000e+000) NOT NULL
, [Arsenic] float DEFAULT (0.0000000000000000e+000) NOT NULL
, [Barium] float DEFAULT (0.0000000000000000e+000) NOT NULL
, [Beryllium] float DEFAULT (0.0000000000000000e+000) NOT NULL
, [Bismuth] float DEFAULT (0.0000000000000000e+000) NOT NULL
, [Cadmium] float DEFAULT (0.0000000000000000e+000) NOT NULL
, [Lead] float DEFAULT (0.0000000000000000e+000) NOT NULL
, [Mercury] float DEFAULT (0.0000000000000000e+000) NOT NULL
, [Nickel] float DEFAULT (0.0000000000000000e+000) NOT NULL
, [Silver] float DEFAULT (0.0000000000000000e+000) NOT NULL
, [Thallium] float DEFAULT (0.0000000000000000e+000) NOT NULL
, [Thorium] float DEFAULT (0.0000000000000000e+000) NOT NULL
, [Tin] float DEFAULT (0.0000000000000000e+000) NOT NULL
, [Titanium] float DEFAULT (0.0000000000000000e+000) NOT NULL
, [Uranium] float DEFAULT (0.0000000000000000e+000) NOT NULL
, [SampleId] ntext NULL
, [LabNumber] ntext NULL
, [Platinum] float DEFAULT (0.0000000000000000e+000) NOT NULL
, CONSTRAINT [PK_Labs] PRIMARY KEY ([LabId])
, CONSTRAINT [FK_Labs_Horses_HorseId] FOREIGN KEY ([HorseId]) REFERENCES [Horses] ([Id]) ON DELETE NO ACTION ON UPDATE NO ACTION
);
...
CREATE INDEX [Labs_IX_Labs_HorseId] ON [Labs] ([HorseId] ASC);
CREATE TRIGGER [fki_Labs_HorseId_Horses_Id] BEFORE Insert ON [Labs] FOR EACH ROW BEGIN SELECT RAISE(ROLLBACK, 'Insert on table Labs violates foreign key constraint FK_Labs_Horses_HorseId') WHERE NEW.HorseId IS NOT NULL AND(SELECT Id FROM Horses WHERE  Id = NEW.HorseId) IS NULL; END;
CREATE TRIGGER [fku_Labs_HorseId_Horses_Id] BEFORE Update ON [Labs] FOR EACH ROW BEGIN SELECT RAISE(ROLLBACK, 'Update on table Labs violates foreign key constraint FK_Labs_Horses_HorseId') WHERE NEW.HorseId IS NOT NULL AND(SELECT Id FROM Horses WHERE  Id = NEW.HorseId) IS NULL; END;
COMMIT;
